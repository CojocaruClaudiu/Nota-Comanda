...
import { Document as PdfJsDocument, Page as PdfJsPage, pdfjs } from "react-pdf";
// Using CDN worker to avoid bundler resolution issues
import * as XLSX from "xlsx";
// Lightweight Buffer.from shim for @react-pdf in browsers
(function(){
  ...
  if (!w.Buffer) {
    w.Buffer = {
      from(input: any, encoding?: string) {
        ...
        if (ArrayBuffer.isView(input)) return new Uint8Array((input as ArrayBufferView).buffer);
...
// Register a Unicode font to render Romanian diacritics correctly in PDF
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: "background.default", p: 3 }}>
      ...
      <Stack spacing={3} direction={{ xs: "column", lg: "row" }} alignItems="stretch" sx={{ minHeight: { lg: "60vh" } }}>
        ...
        <Paper elevation={1} sx={{ p: 3, flex: 1.2, minHeight: { xs: 480, lg: "100%" } }}>
          ...
          <Box sx={{ mt: 2, height: { xs: 480, sm: 560, lg: "100%" }, borderRadius: 1, overflow: "auto" }}>
            <BlobProvider document={pdfDocMemo}>
              {({ url, loading, error }) => (
                <Box sx={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {loading && <Typography variant="body2">Se generează previzualizarea…</Typography>}
                  {error && <Typography color="error">Eroare la generarea PDF</Typography>}
                  {url && (
                    <PdfJsDocument file={url} loading={<Typography variant="body2">Se încarcă…</Typography>}>
                      <PdfJsPage pageNumber={1} width={700} renderAnnotationLayer={false} renderTextLayer={false} />
                    </PdfJsDocument>
                  )}
                </Box>
              )}
